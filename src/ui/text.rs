...
impl TextRenderer {
    ...
    fn reshape_text_run_with_font(
        ...
        let reshaped_glyphs = self.shape_cached(
            &text,
            run.bidi_lvl,
            run.script,
            font,
        ...
        let new_cluster_groups: Arc<[ClusterGroup]> =
            Arc::from(new_cluster_groups_vec);
    ...
    fn shape_cached(
        ...
        let glyphs_vec = self.font_manager.shape_text_with_props(
            text,
            bidi_lvl,
            script,
            font_handle,
    ...
    fn measure_byte_range_from_runs(
        ...
        for (run_index, run) in paragraph_runs.iter().enumerate() {
            ...
            let slices = self.run_slices_for_byte_range(
                run_index,
                run,
    ...
    fn word_wrap_token_ranges(&mut self, 
        paragraph: &str) -> anyhow::Result<Vec<TokenRange>> {
        ...
        for token in paragraph.split_word_bounds() {
            ...
            tokens.push(TokenRange { byte_start, byte_end, width: 0.0});
        }
        ...
        for range in token_ranges {
            if crnt_start.is_none() {
                ...
                crnt_width = if let Some(start) = crnt_start {
                    if start < crnt_end {
                        self.measure_byte_range_from_runs(
                            paragraph_runs,
                            start..crnt_end,
                        )?
                    } else {
            ...
            let would_wrap = crnt_width != 0.0 &&
                crnt_width + range.width >= max_width;
    ...
    fn build_cluster_groups_from_glyphs(
        ...
        while i < glyphs.len() {
            ...
            while glyph_end < glyphs.len()
                && glyphs[glyph_end].cluster as usize == cluster
                {
                    glyph_end += 1;
                }
        ...
        for group in &mut groups {
            ...
            group.byte_end_abs = starts
                .get(pos + 1)
                .copied()
                .unwrap_or(run_byte_end);
            }
    ...
    fn run_slices_for_byte_range(
        ...
        for group in groups.iter() {
            let intersects =
                group.byte_end_abs > byte_range_abs.start &&
    ...
    fn build_run_slices_for_line(
        ...
        for (idx, run) in paragraph_runs.iter().enumerate() {
            ...
            let run_slices = self.run_slices_for_byte_range(
                idx,
                run,
                slice_start..slice_end,
            )?;
    ...
    fn token_ranges_cached(
        ...
        for token in &mut tokens {
            token.width = self.measure_byte_range_from_runs(
                paragraph_runs,
    ...
    fn layout_cached(
        ...
        let line_ranges = self.build_line_ranges(
            text,
            &paragraph_runs,
            &token_ranges,
            options.max_width,
