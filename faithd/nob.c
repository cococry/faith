...
bool build()
{
  ...
  nob_cc_inputs(&cmd, "src/faithd.c", "src/shared.c");
  ...
  nob_cmd_append(&cmd, "-lssl", "-lcrypto");
  nob_cc_inputs(&cmd, "src/client_test_suite.c", "src/shared.c", "src/client/client.c");
...
int main(int argc, char **argv)
{
  ...
  if(command(command_name, &commands, "chain", "", "Generate certificate files \"chain.pem\" and \"pkey.pem\"")) {
    // generate pkey.pem
    {
      Nob_Cmd cmd = {0};
      cmd_append(&cmd, "openssl", "genpkey",
          "-algorithm", "rsa", "-out", "pkey.pem", "-pkeyopt", "rsa_keygen_bits:2048");
      cmd_run(&cmd); 
    }
    // generate chain.pem
    {
      Nob_Cmd cmd = {0};
      cmd_append(&cmd, 
          "openssl", "req", "-x509", "-new", "-key", "pkey.pem", "-days", 
          "36500", "-subj", "/CN=localhost", "-out", "chain.pem");
      cmd_run(&cmd); 
    }
