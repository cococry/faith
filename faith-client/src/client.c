...
#include <faith-proto/codec/signatures.h>
#include <faith-proto/codec/server.h>

#define MAX_QUEUED_EVENTS 512

static bool g_log_enable_tracing = true;
...
static faith_status_code_t pending_command_take(faith_client_t *client,
  ...
  (void)hmdel(client->pending_command_map, *id);
...
static faith_status_code_t client_send_ack_until(faith_client_t *client,
  ...
  envl.type = FAITH_ENVELOPE_EVENT_ACK;
  envl.body = body;
  envl.body_size = body_size;

  _FH_CHECK_RETURN(client_send_envelope_locked(client, &envl));
...
client_handle_event_conversation_created(faith_client_t               *client,
  ...
  _FH_CHECK_RETURN(faith_decode_event_conversation_created(
      event->data, event->data_size, &conv_created));

  char buf[33];
  _FH_CHECK_RETURN(faith_id128_to_hex(conv_created.conversation_id.bytes, buf));
  nob_log(INFO, "[client] Conversation %s was created by the server.", buf);
...
static void *reader(void *arg) {
  ...
  while (atomic_load(&client->connected) && client_is_running(client)) {
    ...
    case FAITH_MSG_ENVL: {
      _FH_CHECK(client_handle_envelope(client, &frame));
      break;
    }
    default:
      break;
...
static faith_status_code_t faith_client_send_hello(faith_client_t *client) {
  ...
  faith_envl_cts_hello_t hello = {0};
  hello.device_id = client->ident.device_id;
  memcpy(hello.public_key, client->ident.public_key,
...
//#define READ_IDENT "ident/c6923af794ef6aa62012539a4ec45d26.bin"
static faith_status_code_t
